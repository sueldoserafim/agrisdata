    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <Tabs defaultValue="containers" className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-6">
          <TabsTrigger value="containers" disabled={!info.acessos_permitidos.includes('containers')}>Containers Ativos</TabsTrigger>
          <TabsTrigger value="documentos" disabled={!info.acessos_permitidos.includes('documentos')}>Pendências Docs</TabsTrigger>
        </TabsList>
                    ...
                    <TableBody>
                      {data.containers.map((item: any) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>{item.gate_in_data ? format(new Date(item.gate_in_data), 'dd/MM/yyyy HH:mm') : 'Pendente'}</TableCell>
                          <TableCell>{item.cut_off ? format(new Date(item.cut_off), 'dd/MM/yyyy') : '-'}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
          ...
          <Card>
            ...
            <CardContent>
              {data.documentos?.length > 0 ? (
                    ...
                    <TableBody>
                      {data.documentos.map((item: any) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>{item.data_emissao ? format(new Date(item.data_emissao), 'dd/MM/yyyy') : '-'}</TableCell>
                          <TableCell>
                ...
                <div className="text-center py-8 text-slate-500">Nenhum documento pendente. Excelente!</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
