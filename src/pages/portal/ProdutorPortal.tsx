    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <Tabs defaultValue="extrato" className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-6">
          <TabsTrigger value="extrato" disabled={!info.acessos_permitidos.includes('extrato')}>
            Extrato
          </TabsTrigger>
          <TabsTrigger value="entregas" disabled={!info.acessos_permitidos.includes('entregas')}>
            Entregas
          </TabsTrigger>
          <TabsTrigger
            value="pagamentos"
        ...
        <TabsContent value="extrato" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Acompanhe os movimentos financeiros da sua produção.
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <TableBody>
                      {data.conta_corrente.map((item: any) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell
                            className={`text-right ${item.valor < 0 ? 'text-red-600' : 'text-green-600'}`}
                          >
                      ...
                      {data.entregas.map((item: any) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="text-right">
                            {item.peso_bruto_kg ? `${item.peso_bruto_kg} kg` : '-'}
                          </TableCell>
                          <TableCell className="text-right">
                            {item.peso_liquido_kg ? `${item.peso_liquido_kg} kg` : '-'}
                          </TableCell>
                          <TableCell className="text-right">
